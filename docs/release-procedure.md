...
and source distribution, publishes them to PyPI with Trusted Publishing, and
...
`dask==YYYY.M.X` package is available on PyPI before smoke-testing and
publishing. The Distributed smoke tests install dependencies from PyPI, so the
